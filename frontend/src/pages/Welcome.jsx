...
export default function Welcome({ onChip }) {
  return (
    <section style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 40px 120px', gap: 32, overflowY: 'auto' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: color.borderSidebar, border: `1px solid ${color.borderSidebar}`, borderRadius: 8, overflow: 'hidden', maxWidth: 720, width: '100%' }}>
        {features.map(({ icon: Icon, title, body }) => (
          <div key={title} style={{ background: color.bgApp, padding: '20px 24px', display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0, 1fr))', gap: 8, maxWidth: 720, width: '100%', marginBottom: 1.5 }}>
        {chips.map((chip) => (
          <HoverButton
            key={chip}
            onClick={() => onChip(chip)}
            style={{ padding: '10px 18px', background: color.bgChip, border: `1px solid ${color.borderCard}`, borderRadius: 6, cursor: 'pointer', fontSize: 13, color: color.gold, whiteSpace: 'nowrap', font: 'inherit', width: '100%' }}

...
def compare_summaries(
    ...
    print(f"  {'Metric':25s}  {'Baseline':>10s}  {'Treatment':>10s}  {'Delta':>10s}  {'Change':>8s}")
    ...
    for key, label in metrics:
        ...
        if key in ("avg_latency_ms", "p95_latency_ms"):
            indicator = "improved" if delta < 0 else "regressed" if delta > 0 else "same"
        else:
            indicator = "improved" if delta > 0 else "regressed" if delta < 0 else "same"

        marker = "+" if indicator == "improved" else "-" if indicator == "regressed" else "="
        print(f"  {label:25s}  {b_val:10.3f}  {t_val:10.3f}  {delta:+10.3f}  {change_str:>8s} {marker}")
...
def print_trend(results_dir: str) -> None:
    ...
    print(f"{'='*80}")
    print(f"  {'Run':30s}  {'Faith':>6s}  {'Relev':>6s}  {'CtxPrec':>7s}  {'CtxRec':>6s}  {'LatP95':>7s}")
